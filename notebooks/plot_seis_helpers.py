...
def interpolate_to_grid(sensor_points, values, theta_grid, phi_grid, verbose=False):
    ...
    try:
        interpolated_values = griddata(sensor_points, sensor_values, grid_points, method="linear", fill_value=np.nan)
        ...
        if np.any(nan_mask):
            ...
            interpolated_values[nan_mask] = griddata(
                sensor_points, sensor_values, grid_points[nan_mask], method="nearest"
            )
    except Exception as e:
        if verbose:
            print(f"Linear interpolation failed, using nearest neighbor: {e}")
        interpolated_values = griddata(sensor_points, sensor_values, grid_points, method="nearest")
...
def create_scatter_trace(
    sensor_x, sensor_y, sensor_z, values, sensor_lon, sensor_lat, global_min, global_max, name=None, marker_size=6
):
    """
...
def create_surface_trace(x_grid, y_grid, z_grid, color_grid, theta_grid, phi_grid, global_min, global_max, name=None):
    """
...
def visualize_wavefield_plotly(npz_file, sensor_csv, index, verbose=False):
    ...
    surface_trace = create_surface_trace(
        x_grid, y_grid, z_grid, color_grid, theta_grid, phi_grid, global_min, global_max
    )
    ...
    layout_settings.update(  # pyrefly: ignore
        {
            "title": dict(text=f"Seismic Wavefield at Time Step {index}", x=0.5, font=dict(size=16)),
            "width": 900,
            "height": 700,
...
def visualize_wavefield_plotly_with_slider(
    npz_file, sensor_csv, n_frames=20, start_index=0, end_index=None, verbose=False
):
    """
    ...
    for i, time_idx in enumerate(frame_indices):
        ...
        frame_data = create_surface_trace(
            x_grid, y_grid, z_grid, color_grid, theta_grid, phi_grid, global_min, global_max, name=f"Time {time_idx}"
        )
    ...
    sliders_dict = {
        ...
        "yanchor": "top",
        "xanchor": "left",
        "currentvalue": {"font": {"size": 20}, "prefix": "Time Index:", "visible": True, "xanchor": "right"},
    ...
    layout_settings.update(  # pyrefly: ignore
        {
            "title": dict(text="Interactive Seismic Wavefield Visualization", x=0.5, font=dict(size=16)),
            "width": 1000,
            "height": 800,
...
def plot_sensor_time_series(npz_file, sensor_csv, index, sensor_idx=None, verbose=False):
    ...
    fig_ts.add_vline(x=index, line_dash="dash", line_color="red", annotation_text=f"Time step {index}")
...
def visualize_wavefield_plotly_scatter(npz_file, sensor_csv, index, verbose=False):
    ...
    inner_sphere = go.Surface(
        ...
        lightposition=dict(
            x=0,
            y=0,
            z=0,  # Light at origin (minimal shadow casting)
        ),
    )
    ...
    layout_settings.update(  # pyrefly: ignore
        {
            "title": dict(text=f"Seismic Wavefield (Scatter) at Time Step {index}", x=0.5, font=dict(size=16)),
            "width": 900,
            "height": 700,
            "margin": dict(l=0, r=0, b=0, t=40),
        }
...
def visualize_wavefield_plotly_scatter_with_slider(
    npz_file, sensor_csv, n_frames=20, start_index=0, end_index=None, verbose=False
):
    """
    ...
    for i, time_idx in enumerate(frame_indices):
        ...
        scatter_trace = create_scatter_trace(
            ...
            global_min,
            global_max,
            name=f"Sensors at time {time_idx}",
        )

        # Create frame with both scatter and sphere
        frames.append(go.Frame(data=[scatter_trace, inner_sphere], name=str(time_idx)))
    ...
    layout_settings.update(  # pyrefly: ignore
        {
            "title": dict(text="Interactive Seismic Wavefield Scatter Visualization", x=0.5, font=dict(size=16)),
            "width": 1000,
            "height": 800,
            "margin": dict(l=0, r=0, b=100, t=100),
            "sliders": [sliders_dict],
...
def plot_wavefield_timeseries_heatmap(npz_file, sensor_csv, verbose=False):
    ...
    # Update layout
    fig.update_layout(
        title=dict(text="Seismic Wavefield Time Series", x=0.5, font=dict(size=16)),
        xaxis=dict(title="Time Step", showgrid=True, gridcolor="lightgray"),
        yaxis=dict(title="Sensor Index", showgrid=True, gridcolor="lightgray"),
        width=1000,
        height=600,
...
def plot_wavefield_timeseries_heatmap_with_geography(
    npz_file, sensor_csv, sort_by="latitude", order="descending", verbose=False
):
    """
    ...
    for i in range(n_sensors):
        ...
        for j in range(n_time):
            row.append(
                f"Time: {j}<br>Sensor: {sort_indices[i]}<br>Lat: {sensor_lat_sorted[i]:.2f}°<br>Lon: {sensor_lon_sorted[i]:.2f}°<br>Amplitude: {wavefield_sorted[i, j]:.3f}"
            )
    ...
    # Update layout
    fig.update_layout(
        title=dict(text=f"Seismic Wavefield Time Series (sorted by {sort_by})", x=0.5, font=dict(size=16)),
        xaxis=dict(title="Time Step", showgrid=True, gridcolor="lightgray"),
        yaxis=dict(
