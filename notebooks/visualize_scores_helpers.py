...
baseline_colormap = plt.cm.Blues  # Blue tones for baselines # pyrefly: ignore
model_colormap = plt.cm.tab20c  # Categorical colors for models # pyrefly: ignore
...
def create_model_color_mapping(all_models):
    ...
    baseline_colors = plt.cm.Blues(np.linspace(0.5, 0.8, len(baseline_models)))  # pyrefly: ignore
    ...
    other_colors = plt.cm.tab20b(np.linspace(0, 1, len(other_models)))  # pyrefly: ignore
    ...
    try:
        ...
        for dataset in datasets:
            dfs[dataset] = dfs[dataset][
                (dfs[dataset]["Model"].notna()) & (dfs[dataset]["Model"] != "") & (dfs[dataset]["avg_score"].notna())
...
def explore_cleaned_data(dfs):
    ...
    for sheet_name, df in dfs.items():
        ...
        print("\nFirst 5 rows:")
        display(df.head())  # noqa: F405 F821

        print("\nBasic statistics (numeric columns only):")
        display(df.describe())  # noqa: F405 F821
...
def create_model_comparison_plots(data, size=(10, 6), globals=None):
    ...
    """
    GLOBAL_MODEL_COLORS, GLOBAL_MODEL_MARKERS, GLOBAL_MODEL_PATTERNS, NEURIPS_DPI = globals  # pyrefly: ignore
    ...
    for i, (dataset, ax) in enumerate(zip(data.keys(), axes)):
        ...
        # Calculate average standard deviation from "E{i} std" columns
        std_cols = [f"E{j + 1} std" for j in range(12)]
        available_std_cols = [col for col in std_cols if col in dataset_data.columns]
        if available_std_cols:
            std_data = dataset_data[available_std_cols]
            std_values = std_data.mean(axis=1, skipna=True).values
        else:
            std_values = None

        bars_pde = axes[i].barh(
            range(len(dataset_data)),
            dataset_data["avg_score"],
            xerr=std_values,
            capsize=3,
    ...
    for model in all_models:
        if "Baseline" in model:
            rect = plt.Rectangle(
                ...
                1,
                1,
                facecolor=GLOBAL_MODEL_COLORS[model],
                edgecolor="black",
                linewidth=1.0,
                hatch=GLOBAL_MODEL_PATTERNS[model],
            )
        else:
            rect = plt.Rectangle((0, 0), 1, 1, facecolor=GLOBAL_MODEL_COLORS[model], hatch=GLOBAL_MODEL_PATTERNS[model])
        legend_elements.append(rect)
        legend_labels.append(model)

    # Add figure title
    fig.suptitle("Model Average Scores Comparison", y=0.95)
...
def create_spider_plots(data, n_top_models=4, size=(10, 6), globals=None):
    ...
    # Task category labels for display
    category_labels = [f"E{i + 1}" for i in range(12)]
    ...
    fig.subplots_adjust(top=0.85, right=0.75, wspace=0.3)
    ...
    for ax, sheet_name in zip(axes, data.keys()):
        ...
        # Determine column naming convention: "E{i}" or "E{i} mean"
        if "E1 mean" in df.columns:
            task_cols = [f"E{i + 1} mean" for i in range(12)]
        else:
        ...
        for idx, (_, model) in enumerate(all_models.iterrows()):
            # Get values for E1-E12 (using appropriate column names)
            values = [model[col] for col in task_cols]
        ...
        all_values = all_models[task_cols].values.flatten()
    ...
    for model in unique_models:
        ...
        if "Baseline" in model:
            line = plt.Line2D(
                [0], [0], color=GLOBAL_MODEL_COLORS[model], linewidth=1.5, linestyle="--", marker=marker, markersize=5
            )
        else:
            line = plt.Line2D(
                [0], [0], color=GLOBAL_MODEL_COLORS[model], linewidth=1.0, linestyle="-", marker=marker, markersize=4
            )
...
def create_compound_bar_plots(data, n_top_models=3, size=(10, 7), globals=None):
    ...
    task_labels = [f"E{i + 1}" for i in range(12)]
    ...
    for sheet_name in data.keys():
        ...
        # Determine std column naming convention
        if "E1 mean" in df.columns:
            std_cols = [f"E{i + 1} std" for i in range(12)]
        else:
            std_cols = [f"E{i + 1} std" for i in range(12)] if "E1 std" in df.columns else None

        for idx, (task_col, task_label) in enumerate(zip(task_cols, task_labels)):
            ax = axes[idx]
            std_col = std_cols[idx] if std_cols and std_cols[idx] in df.columns else None

            # Get top 3 models for this task
            task_data = df[["Model", task_col]].dropna()
            top_models = task_data.nlargest(n_top_models, task_col)

            # Check if baseline is already in top 3
            if baseline_name in top_models["Model"].values:
                top_models = task_data.nlargest(n_top_models + 1, task_col)
                models = list(top_models["Model"])
                scores = list(top_models[task_col])
                # Get std values for these models
                if std_col:
                    std_values = [
                        df.loc[df["Model"] == m, std_col].values[0] if m in df["Model"].values else 0 for m in models
                    ]
                else:
                    std_values = None
            else:
                models = [baseline_name] + list(top_models["Model"])
                scores = [baseline[task_col]] + list(top_models[task_col])
                # Get std values for these models
                if std_col:
                    std_values = [
                        df.loc[df["Model"] == m, std_col].values[0] if m in df["Model"].values else 0 for m in models
            ...
            # Create bar plot with patterns and error bars
            bars = ax.bar(
                range(len(models)),
                scores,
                color=colors,
            ...
            for bar, score in zip(bars, scores):
                if pd.notna(score):
                    ...
                    if height >= 0:
                        ...
                        va = "top"
                        y_offset = -1
                    ax.text(bar.get_x() + bar.get_width() / 2.0, height + y_offset, f"{score:.1f}", ha="center", va=va)
...
def create_compact_compound_bar_plots(data, n_top_models=3, size=(10, 5), globals=None):
    ...
    for fig_idx, sheet_name in enumerate(data.keys()):
        ...
        for task_col in task_cols:
            task_data = df[["Model", task_col]].dropna()
            extended_top = task_data.nlargest(n_top_models + 1, task_col)
...
def create_ultra_compact_bar_plots(data, n_top_models=3, size=(10, 5), use_patterns=True, globals=None):
    ...
    for fig_idx, sheet_name in enumerate(data.keys()):
        ...
        for idx, (task_col, task_label) in enumerate(zip(task_cols, task_labels)):
            ...
            top_models = non_baseline_data.nlargest(n_top_models, task_col)
            ...
            baseline_score = baseline[task_col]
            ...
            for i, (bar, model, score) in enumerate(zip(bars, models, scores)):
                if pd.notna(score):
                    ...
                    ax.text(
                        bar.get_x() + bar.get_width() / 2.0, y_pos, model, rotation=90, ha="center", va=va, fontsize=6
                    )
...
def create_task_performance_heatmaps(data, size=(10, 6), globals=None):
    ...
    for ax, sheet_name in zip(axes, data.keys()):
        ...
        # heatmap
        pde_task_data = df[["Model"] + task_cols].set_index("Model")
        ...
        ax.set_title(f"{sheet_name}: Task Performance Heatmap")
...
def create_interactive_task_comparison(data):
    ...
    # Determine column naming convention: "E{i}" or "E{i} mean"
    first_df = data[list(data.keys())[0]]
    if "E1 mean" in first_df.columns:
        task_cols = [f"E{i + 1} mean" for i in range(12)]
    else:
        task_cols = [f"E{i + 1}" for i in range(12)]
    ...
    fig = make_subplots(
        rows=3,
        cols=4,
        subplot_titles=[f"Task {label}" for label in task_labels],
    ...
    for idx, (task_col, task_label) in enumerate(zip(task_cols, task_labels)):
        ...
        for sheet_name in data.keys():
            scores = data[sheet_name][["Model", task_col]].dropna()
            fig.add_trace(
                go.Bar(
                    x=scores["Model"],
                    y=scores[task_col],
    ...
    fig.update_layout(
        height=1000, title_text="Model Performance Comparison Across All Tasks", showlegend=True, barmode="group"
    )
...
def statistical_analysis(data, size=(10, 8), globals=None):
    ...
    for sheet_num, sheet_name in enumerate(data.keys()):
        ...
        task_data = df[task_cols]

        # Rename columns for display
        task_data_display = task_data.copy()
        task_data_display.columns = task_labels

        # Box plot of task score distributions
        print(type(axes))
        task_data_display.plot(kind="box", ax=axes[sheet_num, 0], rot=45)
        ...
        print("1")
        model_scores_ode = df[task_cols].values.flatten()
        model_names_ode = np.repeat(df["Model"].values, len(task_cols))
        task_names_ode = np.tile(task_labels, len(df))

        ode_long_df = pd.DataFrame(
            {"Model": model_names_ode, "Task": task_names_ode, "Score": model_scores_ode}
        ).dropna()
...
def create_model_rankings(data, globals=None):
    ...
    fig.update_layout(height=600, showlegend=False, title_text="Model Rankings by Average Score")
...
def create_model_rankings_matplotlib(data, size=(6.5, 3.5), globals=None):
    ...
    for sheet_num, sheet_name in enumerate(data.keys()):
        plot_ranking_scatter(
            axes[sheet_num], data[sheet_name], f"{sheet_name} Model Rankings", "viridis", scatter_size, fig, globals
        )
...
def plot_ranking_scatter(ax, df, title, colormap, scatter_size, fig, globals=None):
    ...
    for idx, row in ranked.iterrows():
        ...
        ax.scatter(
            ...
            linewidth=0.5,
            marker=marker,
            vmin=ranked["avg_score"].min(),
            vmax=ranked["avg_score"].max(),
        )

    # Add colormap
    sm = plt.cm.ScalarMappable(
        cmap=colormap, norm=plt.Normalize(vmin=ranked["avg_score"].min(), vmax=ranked["avg_score"].max())
    )
    sm.set_array([])
...
def create_latex_performance_tables(data):
    ...
    for sheet_name, df in data.items():
        ...
        latex_str += "\\begin{{adjustbox}}{{width=\\textwidth}}\n"
        latex_str += f"\\begin{{tabular}}{{l|r|{'r' * 12}}}\n"
        latex_str += "\\hline\n"
        latex_str += "\\textbf{{Model}} & \\textbf{{Avg Score}} & "
